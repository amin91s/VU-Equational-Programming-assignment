...
nextGeneration w = S.union (survive w) (born w)

-- | Helper functions
deadCells :: World -> World
deadCells w = S.difference (S.foldr (S.union . (\c -> neighbors c)) S.empty w) w

survive :: World -> World
survive w = S.filter (\c -> (neighborCountIn c w == 2) || (neighborCountIn c w == 3)) w

born :: World -> World
born w = S.filter (\c -> neighborCountIn c w == 3) (deadCells w)
